...
class UtilityEdittingV2:
    ...
    def run(self):
        ...
        self.point_layer = QgsVectorLayer("C:/Users/SAM/Desktop/Uni/Internship/15JanTask/Point_Features.shp", "Points", "ogr")
    ...
    def connect_signals(self, layer):
        ...
        self.layer = layer
        if self.layer.type() == self.point_layer.type():
            ##Point layer listeners
            QgsMessageLog.logMessage(self.layer.name())
            self.layer.featureAdded.connect(self.feat_added)
        elif self.layer.type() == self.pipe_layer.type():
            ##Pipe layer listeners
            QgsMessageLog.logMessage(self.layer.name())
            self.layer.featureAdded.connect(self.pipe_added)
            
    def feat_added(self, fid):
        """Check if new point is splitting a line and if so create a new pipe line and adjust geometries."""
        QgsMessageLog.logMessage("Point added.")
        QgsMessageLog.logMessage(self.layer.name())
        
    def pipe_added (self, fid):
        ...
        for feature in pipe_features:
            if feature.id() == fid:
                ...
                QgsMessageLog.logMessage(geom.asLine())
